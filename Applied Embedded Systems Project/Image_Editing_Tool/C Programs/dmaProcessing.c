...
void memdump(void* virtual_address, int byte_count) {
    ...
    int offset;
    FILE *fh = fopen("output_grey_last.bin","wb");
    /*
    ...
    printf("\n");
    fwrite(p,1,byte_count,fh);
    printf("Output bin file created!\n");
...
int main() {
    ...
    unsigned int* virtual_address = mmap(NULL, 65544, PROT_READ | PROT_WRITE, MAP_SHARED, dh, 0x40400000); // Memory map AXI Lite register block
    unsigned int* virtual_source_address  = mmap(NULL, 65544, PROT_READ | PROT_WRITE, MAP_SHARED, dh, 0x10000000); // Memory map source address
    unsigned int* virtual_destination_address = mmap(NULL, 65544, PROT_READ | PROT_WRITE, MAP_SHARED, dh, 0x10000000); 

 //   printf("Source memory block:      "); memdump(virtual_source_address, 256);
//    printf("Destination memory block: "); memdump(virtual_destination_address, 256);
    ...
    dma_set(virtual_address, S2MM_DESTINATION_ADDRESS, 0x10000000); // Write destination address
    dma_s2mm_status(virtual_address);

    printf("Writing source address...\n");
    dma_set(virtual_address, MM2S_START_ADDRESS, 0x10000000); // Write source address
    dma_mm2s_status(virtual_address);
    ...
    dma_set(virtual_address, S2MM_LENGTH, 65544);
    dma_s2mm_status(virtual_address);

    printf("Writing MM2S transfer length...\n");
    dma_set(virtual_address, MM2S_LENGTH, 65544
);
    dma_mm2s_status(virtual_address);
    ...
    printf("Output bin file creation...\n: "); memdump(virtual_destination_address, 65544
);
}
